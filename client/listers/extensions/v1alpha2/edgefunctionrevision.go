...
type EdgeFunctionRevisionLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.EdgeFunctionRevision, err error)
	// Get retrieves the EdgeFunctionRevision from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.EdgeFunctionRevision, error)
...
// List lists all EdgeFunctionRevisions in the indexer.
func (s *edgeFunctionRevisionLister) List(selector labels.Selector) (ret []*v1alpha2.EdgeFunctionRevision, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.EdgeFunctionRevision))
	})
	return ret, err
}

// Get retrieves the EdgeFunctionRevision from the index for a given name.
func (s *edgeFunctionRevisionLister) Get(name string) (*v1alpha2.EdgeFunctionRevision, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("edgefunctionrevision"), name)
	}
	return obj.(*v1alpha2.EdgeFunctionRevision), nil
}
